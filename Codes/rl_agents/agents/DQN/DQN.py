...
class DQN_agent:
    ...
    TODO: update so it accepts a history of observation
    """
    ...
    def __init__(
        ...
        self.mem_state = np.zeros(
            shape=(self.mem_size, *self.state_dim), dtype=np.float32
        )
    ...
    def learn(self):
        ...
        learning_index = np.random.choice(index, self.batch_size, np.int32)
        batch_index = np.arange(self.batch_size, dtype=np.int32)

        state_batch = torch.tensor(self.mem_state[learning_index])
        action_batch = torch.tensor(self.mem_action[learning_index])
        reward_batch = torch.tensor(self.mem_reward[learning_index])
        done_batch = torch.tensor(self.mem_done[learning_index])
        next_state_batch = torch.tensor(self.mem_next_state[learning_index])
