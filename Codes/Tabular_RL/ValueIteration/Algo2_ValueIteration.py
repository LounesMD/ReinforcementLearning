...
def IterationValeur(etats, actions, transitions, retour, gamma, epsilon):
            ...
            for action in actionsPossibles:
                ...
                for etatp in etats:
                    v += (transitions[etat][action][etatp]) * (
                        retour[etat][action][etatp] + gamma * ValeurK[etatp]
                    )
                l.append(v)
    ...
    for etatp in etats:
        ...
        for action in actionsPossibles:
            v = 0
            for j in etats:
                v += transitions[etatp][action][j] * (
                    retour[etatp][action][j] + gamma * ValeurK[j]
                )
            l.append([action, v])
...
print(
    "Avec l'algotithme d'itération sur les valeurs, on trouve comme valeur optimale pour chaque état :"
)
print(politique[1])
