    ...
    while comp:
        ...
        while cmp > epsilon * ((1 - gamma) / (2 * gamma)):
            ...
            for etat in etats:
                v = 0
                for etatp in etats:
                    v += (transitions[etat][politique[etat]][etatp]) * (
                        retour[etat][politique[etat]][etatp] + gamma * ValeurI[etatp]
                    )
                ValeurI1[etat] = v
        ...
        for etat in etats:
            ...
            for action in actionsPossibles:
                v = 0
                for etatp in etats:
                    v += transitions[etat][action][etatp] * (
                        retour[etat][action][etatp] + gamma * ValeurI[etatp]
                    )
...
politique = IterationPolitiques(
    etats, actions, transition_taxi(), retour_taxi(), 0.9, 0.01
)
print(
    "Avec l'algotithme d'itération sur les politiques, on trouve comme valeur optimale pour chaque état :"
)
print(politique[1])
