            ...
            for action in actions:
                ...
                for i in range(n):
                    ...
                    while not final(e):
                        observation = (
                            list()
                        )  # In observation we stock the tupple (reward,state) after executing the action b in e
                        ...
                        action = choices(actionsPossibles, d, k=1)[
                            0
                        ]  # We stock in `action` the action to do

                        # If we use a deterministic choice function (such as greedy)
                        # action = decision(Q , actionsPossibles , s[t] , 1 )
                        reward, e = execution(
                            e, action
                        )  # We execute the action in the state e and observe the new state 'e_state' and the reward
                        sum += (gamma**j) * reward
